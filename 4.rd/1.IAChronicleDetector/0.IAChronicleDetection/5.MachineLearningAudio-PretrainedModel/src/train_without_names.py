...
def segment_generator(pairs: List[Dict], label2id: Dict, chronicle_step: float = 5.0, background_step: float = 10.0):
    for pair in pairs:
        ...
        with open(pair['tc_path'], 'r', encoding='utf-8') as f:
            for line in f:
                ...
                if match:
                    ...
                    # Sample segments from the chronicle - Label is always "chronique"
        ...
        # Add background segments - Increased frequency for balance
        chronicle_intervals.sort()
        last_end = 0
        for start, end in chronicle_intervals:
            if start > last_end + 5.0:
                # We sample more background to balance the classes
...
def compute_metrics(p: EvalPrediction):
    ...
    f1 = f1_score(labels, preds, average='binary') # Use binary for 2 classes
...
def train(epochs=10, tags=None, chronicle_step=5.0, background_step=10.0, background_percent=None):
    hardware_info = "CPU"
    ...
    if background_percent is not None:
        ...
        print(f"Counts before balancing - Background: {n_bg}, Chronique: {n_chr}")
