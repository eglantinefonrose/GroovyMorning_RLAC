...
MODEL_NAME = "MIT/efficientnet-b0-audioset"
OUTPUT_DIR = "./model_output_cnn"
SAMPLING_RATE = 16000
MAX_DURATION = 10.0

def predict_single_file(audio_path, model_type="cnn"):
    loader = ModelLoader()
    if model_type == "cnn":
        loader.load_cnn(OUTPUT_DIR)
...
def train(epochs=10, background_percent=None, model_path=None, max_samples=8000, chronicle_step=10.0, background_step=20.0):
    cleanup_cache()
    run_name = f"cnn-chronicle-detector-{datetime.now().strftime('%d/%m-%H:%M')}"
    ...
    wandb.init(project="RLAC-Audio", name=run_name, tags=["cnn"], config={"model": model_path or MODEL_NAME, "epochs": epochs})
    ...
    loader = ModelLoader()
    model, extractor = loader.init_cnn(model_path or MODEL_NAME, label2id, id2label)
