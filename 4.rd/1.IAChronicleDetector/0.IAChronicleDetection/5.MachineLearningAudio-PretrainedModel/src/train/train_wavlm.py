import os
import re
import json
import torch
import librosa
import numpy as np
import socket
import argparse
import shutil
import random
import sys
from pathlib import Path
from datetime import datetime

# Add the parent directory (src) to sys.path to allow importing modules from it
sys.path.append(str(Path(__file__).resolve().parent.parent))

from pydub import AudioSegment
import evaluate
from sklearn.metrics import f1_score
import wandb
from datasets import Dataset, Features, Value, Sequence
from transformers import (
    WavLMForSequenceClassification,
    Wav2Vec2FeatureExtractor,
    TrainingArguments,
    Trainer,
    EvalPrediction,
    TrainerCallback,
    EarlyStoppingCallback
)
from typing import List, Dict
from models_loader import ModelLoader

# --- Configuration des chemins et paramètres ---
AUDIO_ROOT = "../../../@assets/0.media/audio"
TIMECODE_ROOT = "../../../@assets/2.humanOutputs/1.timecode-segments/2.audio-analyse/timecode_chroniques"
MODEL_NAME = "microsoft/wavlm-base-plus-sv"  # Modèle de base WavLM
OUTPUT_DIR = "./model_output_wavlm"          # Dossier de sortie pour le modèle entraîné
SAMPLING_RATE = 16000                        # Taux d'échantillonnage requis par WavLM
MAX_DURATION = 10.0                          # Durée maximale de chaque segment audio (secondes)

def predict_single_file(audio_path, model_type="wavlm"):
    """Utilitaire pour tester le modèle sur un fichier audio unique après l'entraînement."""
    loader = ModelLoader()
    if model_type == "wavlm":
        loader.load_wavlm(OUTPUT_DIR)
    
    model, _ = loader.get_model(model_type)
    if not model:
        print(f"Error: Model {model_type} not found at {OUTPUT_DIR}")
        return

    from predict import predict
    results = predict(audio_path, model_type=model_type, model_dir=OUTPUT_DIR)
    print(f"\nResults for {model_type}:")
    print(json.dumps(results, indent=4, ensure_ascii=False))

def cleanup_cache():
    """Nettoie le cache temporaire des datasets Hugging Face pour libérer de l'espace disque."""
    cache_path = Path.home() / ".cache" / "huggingface" / "datasets" / "generator"
    if cache_path.exists():
        shutil.rmtree(cache_path, ignore_errors=True)

def parse_timecode(tc_str: str) -> float:
    """Convertit un format [HH:MM:SS:mmm] en secondes (float)."""
    parts = tc_str.strip("[] ").split(":")
    if len(parts) == 4:
        h, m, s, ms = map(int, parts)
        return h * 3600 + m * 60 + s + ms / 1000.0
    return 0.0

def load_dataset_pairs():
    """Scanne les dossiers pour associer les fichiers audio aux fichiers de timecodes correspondants."""
    data = []
    if not os.path.exists(TIMECODE_ROOT): return []
    radios = [d for d in os.listdir(TIMECODE_ROOT) if os.path.isdir(os.path.join(TIMECODE_ROOT, d))]
    for radio in radios:
        news_dir = os.path.join(TIMECODE_ROOT, radio, "news")
        if not os.path.exists(news_dir): continue
        for tc_file in os.listdir(news_dir):
            if tc_file.endswith("_new_from_audio.txt"):
                date_str = tc_file.replace("_new_from_audio.txt", "")
                audio_folder = os.path.join(AUDIO_ROOT, radio, date_str)
                if not os.path.exists(audio_folder): continue
                audio_files = os.listdir(audio_folder)
                # Priorité aux fichiers déjà tronqués (_trimmed)
                audio_file = next((f for f in audio_files if "_trimmed" in f and (f.endswith(".mp3") or f.endswith(".m4a"))), None)
                if not audio_file:
                    audio_file = next((f for f in audio_files if (f.endswith(".mp3") or f.endswith(".m4a")) and not "_trimmed" in f), None)
                if audio_file:
                    data.append({"audio_path": os.path.join(audio_folder, audio_file), "tc_path": os.path.join(news_dir, tc_file)})
    return data

def estimate_counts(pairs, chronicle_step, background_step):
    """Estime le nombre de segments 'chronique' et 'background' disponibles pour équilibrer le dataset."""
    total_chr, total_bg = 0, 0
    for pair in pairs:
        intervals = []
        try:
            with open(pair['tc_path'], 'r', encoding='utf-8') as f:
                for line in f:
                    m = re.search(r"\[(.*?)\]\s*-\s*\[(.*?)\]", line)
                    if m:
                        s, e = parse_timecode(m.group(1)), parse_timecode(m.group(2))
                        if e > s:
                            intervals.append((s, e))
                            total_chr += int(max(0, (e - s - 2.0) // chronicle_step) + 1)
            intervals.sort()
            last = 0
            for s, e in intervals:
                if s > last + 5.0: total_bg += int(max(0, (s - 5.0 - last) // background_step) + 1)
                last = max(last, e)
        except: continue
    return total_chr, total_bg

def segment_generator(pairs, label2id, chronicle_step=10.0, background_step=20.0, bg_keep_prob=1.0, chr_keep_prob=1.0, max_samples=10000):
    """Générateur qui extrait des segments audio à la volée pour l'entraînement."""
    count = 0
    for pair in pairs:
        if count >= max_samples: break
        try: audio = AudioSegment.from_file(pair['audio_path'])
        except: continue
        intervals = []
        
        # 1. Extraction des segments marqués comme "chroniques"
        with open(pair['tc_path'], 'r', encoding='utf-8') as f:
            for line in f:
                if count >= max_samples: break
                m = re.search(r"\[(.*?)\]\s*-\s*\[(.*?)\]", line)
                if m:
                    s, e = parse_timecode(m.group(1)), parse_timecode(m.group(2))
                    if e <= s: continue
                    intervals.append((s, e))
                    # On découpe la chronique en segments de MAX_DURATION
                    for seg_s in np.arange(s, e - 2.0, chronicle_step):
                        if count >= max_samples: break
                        if random.random() > chr_keep_prob: continue
                        seg_e = min(seg_s + MAX_DURATION, e)
                        seg = audio[seg_s*1000:seg_e*1000]
                        samples = np.array(seg.get_array_of_samples()).astype(np.float32)
                        # Conversion stéréo -> mono si nécessaire
                        if seg.channels == 2: samples = samples.reshape((-1, 2)).mean(axis=1)
                        # Normalisation
                        samples /= np.max(np.abs(samples)) if np.max(np.abs(samples)) > 0 else 1
                        # Ré-échantillonnage à 16kHz
                        if seg.frame_rate != SAMPLING_RATE: samples = librosa.resample(samples, orig_sr=seg.frame_rate, target_sr=SAMPLING_RATE)
                        yield {"audio_array": samples, "label": label2id["chronique"]}
                        count += 1
        
        # 2. Extraction des segments "background" (tout ce qui n'est pas chronique)
        intervals.sort()
        last = 0
        for s, e in intervals:
            if count >= max_samples: break
            if s > last + 5.0:
                for bg_s in np.arange(last, s - 5.0, background_step):
                    if count >= max_samples: break
                    if random.random() > bg_keep_prob: continue
                    bg_e = min(bg_s + MAX_DURATION, s)
                    seg = audio[bg_s*1000:bg_e*1000]
                    samples = np.array(seg.get_array_of_samples()).astype(np.float32)
                    if seg.channels == 2: samples = samples.reshape((-1, 2)).mean(axis=1)
                    samples /= np.max(np.abs(samples)) if np.max(np.abs(samples)) > 0 else 1
                    if seg.frame_rate != SAMPLING_RATE: samples = librosa.resample(samples, orig_sr=seg.frame_rate, target_sr=SAMPLING_RATE)
                    yield {"audio_array": samples, "label": label2id["background"]}
                    count += 1
            last = max(last, e)

def compute_metrics(p):
    """
    Calcule les métriques de performance.
    - Accuracy : Pourcentage de prédictions correctes globales.
    - F1-Score : Moyenne harmonique entre Précision et Rappel. 
      C'est la métrique clé ici car elle équilibre la détection des chroniques (Rappel) 
      et la fiabilité des prédictions (Précision), évitant d'être biaisé par un dataset déséquilibré.
    """
    preds = np.argmax(p.predictions, axis=1)
    f1 = f1_score(p.label_ids, preds, average='binary') 
    return {"accuracy": (preds == p.label_ids).astype(float).mean(), "f1": f1}

class UnfreezeCallback(TrainerCallback):
    """
    Callback pour le fine-tuning progressif (Gradual Unfreezing).
    
    Concepts clés :
    - La Tête de Classification : C'est la couche finale "neuve" ajoutée au modèle. 
      Alors que WavLM comprend le son de manière générale, cette tête est spécifique 
      à ton projet : elle traduit les concepts abstraits de WavLM en deux choix concrets 
      pour toi : "chronique" ou "background".
    
    Principe du dégèle : 
    1. Au départ, l'encodeur WavLM (qui contient les connaissances pré-entraînées) est gelé.
       On n'entraîne que la tête de classification pour ne pas "casser" les poids pré-entraînées.
    2. Après 2 époques, on "dégèle" l'encodeur. La tête de classification étant déjà stabilisée,
       le modèle peut maintenant affiner ses couches internes pour s'adapter spécifiquement
       aux nuances de tes données audio (acoustique des radios, types de voix, etc.).
    """
    def on_epoch_begin(self, args, state, control, **kwargs):
        if state.epoch >= 2:
            model = kwargs.get("model")
            if model and hasattr(model, "wavlm"):
                print("Unfreezing WavLM encoder...")
                # Autorise la mise à jour des paramètres de l'encodeur WavLM
                for param in model.wavlm.feature_extractor.parameters():
                    param.requires_grad = True

def train(epochs=10, background_percent=None, model_path=None, max_samples=8000):
    """Fonction principale d'entraînement."""
    cleanup_cache()
    run_name = f"wavlm-chronicle-detector-{datetime.now().strftime('%d/%m-%H:%M')}"
    pairs = load_dataset_pairs()
    label2id, id2label = {"background": 0, "chronique": 1}, {0: "background", 1: "chronique"}
    
    # Équilibrage automatique des classes
    bg_prob, chr_prob = 1.0, 1.0
    if background_percent is not None:
        n_c, n_b = estimate_counts(pairs, 10.0, 20.0)
        target = background_percent / 100.0
        if n_b > 0 and n_c > 0:
            if n_b/(n_b+n_c) < target: chr_prob = (n_b*(1-target)/target)/n_c
            else: bg_prob = (n_c*target/(1-target))/n_b

    # Initialisation de la surveillance avec Weights & Biases (WandB)
    wandb.init(project="RLAC-Audio", name=run_name, tags=["wavlm"], config={"model": model_path or MODEL_NAME, "epochs": epochs, "max_samples": max_samples})
    
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    with open(os.path.join(OUTPUT_DIR, "label_mapping.json"), "w") as f: 
        json.dump({"label2id": label2id, "id2label": id2label}, f)

    # Définition de la structure du dataset
    features = Features({"audio_array": Sequence(Value("float32")), "label": Value("int32")})
    
    # Création du dataset via le générateur
    ds = Dataset.from_generator(
        segment_generator, 
        gen_kwargs={"pairs": pairs, "label2id": label2id, "bg_keep_prob": bg_prob, "chr_keep_prob": chr_prob, "max_samples": max_samples}, 
        features=features
    ).train_test_split(test_size=0.1) # Division 90% train / 10% test
    
    # Chargement du modèle et de l'extracteur de caractéristiques
    loader = ModelLoader()
    model, extractor = loader.init_wavlm(model_path or MODEL_NAME, label2id, id2label)
    
    # On gèle l'encodeur au début pour n'entraîner que la tête de classification
    model.freeze_feature_encoder()

    # Fonction de prétraitement (conversion audio brut -> caractéristiques WavLM)
    def preprocess(ex): return extractor(ex["audio_array"], sampling_rate=SAMPLING_RATE, max_length=int(SAMPLING_RATE*MAX_DURATION), truncation=True, padding="max_length")
    
    # Application du prétraitement sur tout le dataset
    ds = ds.map(preprocess, batched=True, remove_columns=["audio_array"])
    
    # Configuration des arguments d'entraînement
    args = TrainingArguments(
        output_dir=OUTPUT_DIR,
        eval_strategy="epoch",
        save_strategy="epoch",
        learning_rate=5e-5,
        per_device_train_batch_size=1,       # Taille de batch (1 car audio lourd)
        per_device_eval_batch_size=1,        # Réduit pour éviter OOM sur Mac
        gradient_accumulation_steps=16,      # Simule un batch de 16 (1 * 16)
        num_train_epochs=epochs,
        load_best_model_at_end=True,         # Recharge le meilleur modèle à la fin
        metric_for_best_model="f1",
        report_to="wandb",
        save_total_limit=1,                  # Ne garde que le meilleur checkpoint
        save_only_model=True,
        fp16=torch.cuda.is_available(),      # Utilisation du float16 si GPU disponible
        dataloader_pin_memory=False,         # Désactivé pour compatibilité MPS (Mac)
        eval_accumulation_steps=1            # Déplace immédiatement les prédictions sur CPU
    )
    
    # Initialisation de l'objet Trainer de Hugging Face
    trainer = Trainer(
        model=model, 
        args=args, 
        train_dataset=ds["train"], 
        eval_dataset=ds["test"], 
        compute_metrics=compute_metrics, 
        callbacks=[UnfreezeCallback(), EarlyStoppingCallback(early_stopping_patience=3)]
    )
    
    # Lancement de l'entraînement
    trainer.train()
    
    # Sauvegarde finale du modèle et de l'extracteur
    trainer.save_model(OUTPUT_DIR)
    extractor.save_pretrained(OUTPUT_DIR)

    # Nettoyage des dossiers de checkpoints temporaires pour gagner de la place
    print("Cleaning up checkpoints...")
    for path in Path(OUTPUT_DIR).glob("checkpoint-*"):
        if path.is_dir():
            shutil.rmtree(path)

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--epochs", type=int, default=10)
    parser.add_argument("--background_percent", type=float, default=None)
    parser.add_argument("--predict", type=str, help="Path to audio file for prediction")
    parser.add_argument("--model_path", type=str, help="Path to base model for fine-tuning")
    args = parser.parse_args()
    
    if args.predict:
        predict_single_file(args.predict)
    else:
        train(epochs=args.epochs, background_percent=args.background_percent, model_path=args.model_path)